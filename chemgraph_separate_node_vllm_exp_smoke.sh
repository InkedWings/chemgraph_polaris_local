...
# Run on a ChemGraph worker node while using a vLLM server on a separate node.
...
BASE_URL="${VLLM_BASE_URL:-http://$VLLM_NODE:$VLLM_PORT/v1}"
PROXY="${PROXY_URL:-http://proxy.alcf.anl.gov:3128}"
NO_PROXY_LOCAL="${NO_PROXY:-127.0.0.1,localhost,::1}"
BASE_URL_AUTHORITY="${BASE_URL#*://}"
VLLM_HOST="${BASE_URL_AUTHORITY%%[:/]*}"
if [[ -n "$VLLM_HOST" && "$NO_PROXY_LOCAL" != *"$VLLM_HOST"* ]]; then
  NO_PROXY_LOCAL="$NO_PROXY_LOCAL,$VLLM_HOST"
fi
